...
<div align="center">
  ...
  <p>Что нового в v1.1.6 : 
    <p>
      Новый год — новые изменения. Изменён состав программы и переработано меню ( если все файлы были в одной папке, а меню было единым, теперь всё по-другому). 
    <p>
    <p>
      1: Меню разложено на подчасти, если все функции были в 1 файле меню (adb.bat), теперь находятся по отдельности. (Например, функция подменю «ИНФОРМАЦИЯ ОБ УСТРОЙСТВЕ» была в файле adb.bat, теперь функция находятся в отдельном файле info.bat, а в adb.bat создана символическая ссылка на этот файл.)
      <p>
<p>Это было сделано, потому что из-за большого количества команд и подменю случались баги (вылет всего меню или вылет при выполнении некоторых команд). Теперь программы находятся в отдельных папках, а подменю вынесены в модули (для каждой команды свой файл).<p>
  <p>
    2: Всё разложено по полочкам и наведен порядок (чтобы легче ориентироваться в файлах). Скорее опробуйте новую версию, чтобы увидеть все изменения! Спасибо.
    <p>
      <p>
...
<a href="https://github.com/FURRY-LOVE/SUPER-ADB/blob/main/ADB.BAT">
  ...
  <a href="https://github.com/FURRY-LOVE/SUPER-ADB/blob/main/cotologfail.txt">
  <img src="https://img.shields.io/badge/Ознакомиться_с_ деревом_каталогов-00aa00?style=for-the-badge" alt=" дерево/каталогов">
</a>
</p>
...
Перед установкой, пожалуйста, отключите антивирус
добавьте папку, в которую будет установлена сборка, в исключения антивируса.
программа неимеет вирусов (программы модифицырованые поэтому он жалуется)
ПРОГРАММА СОБРАНА ИЗ ИСХОДНИКОВ И ОТКРЫТОГО КОДА !
Сборка защищена лицензией General Public License v2 (GPL-2.0)
Сборка была и будет бесплатной но вы можите поддержать автора
